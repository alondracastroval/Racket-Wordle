...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |hw9-problem1 Final Submission (1)|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (value-in-list? x lox p?)
  (local [;inside-list? : X -> Boolean
          ; checks if the supplied value fits the predicate
          (define (inside-list? elem)
            (p? x elem))]
    (ormap inside-list? lox)))
...
(define (score guess1 correct)
  (local [; make-statuspair : String String -> LetterStatusPair
          ; creates a LSP based on whether the string is corect, misplaced, or wrong
          (define (make-statuspair letter1 letter2)
            (cond
              [(string=? letter1 letter2) (make-pair letter1 LS-RIGHT)] 
              [(string-in-list? letter1 (explode correct))
               (make-pair letter1 LS-MISPLACED)]
              [else (make-pair letter1 LS-WRONG)]))]
    (mymap2 (explode guess1) (explode correct) make-statuspair))) 
...
(define (partial-guess->full-guess str length)
  (local [;; make-spaces : String -> List
          ;; makes a list of spaces based on length of string
          ; and provided length
          (define (make-spaces str)
            (build-list (- length (string-length str)) (λ (_) " ")))]
    (implode (append (explode str) (make-spaces str)))))
...
(define (key-wordle ws key valid len)
  (cond
    [(key=? key "\b") (backspace-fun ws len)]
    [(key=? key "\r") (return-fun ws valid len)]
    [(not (number? (string->number key))) (other-fun ws key len)]
    [else ws]))
  
 
; backspace-fun : WS Nat -> WS
; returns a worldlestate when backspace is pressed

(check-expect (backspace-fun WS-START 3) WS-START)
(check-expect (backspace-fun WS-TYPE1 3) WS-START)
(check-expect (backspace-fun WS-TYPE2 3) WS-TYPE1)
(check-expect (backspace-fun WS-TYPE3 3) WS-TYPE2)
(check-expect (backspace-fun WS-TYPE-BAD 3) WS-TYPE2)
(check-expect (backspace-fun WS-WORD1 3) WS-WORD1)


(define (backspace-fun ws len)
  (if (and
       (> (string-length (wordle-current ws)) 0)
       (<= (string-length (wordle-current ws)) len))       
...
;return-fun : WS [List-of String] Nat -> WS
; returns a world state when enter key is pressed

(check-expect (return-fun WS-START LITTLE-WORDS 3) WS-START)
(check-expect (return-fun WS-TYPE1 LITTLE-WORDS 3) WS-TYPE1)
(check-expect (return-fun WS-TYPE3 LITTLE-WORDS 3) WS-WORD1)



(define (return-fun ws valid len)
  (if
   (and (= len (string-length (wordle-current ws)))
        (string-in-list? (wordle-current ws) LITTLE-WORDS))
   (make-wordle (append (list (wordle-current ws)) (wordle-past ws))
                "")
   ws)) 
...
;other-fun : WS 1String Nat -> WS
; types letter into the program if
; there's space

(check-expect
 (other-fun WS-START "C" 3)
 WS-TYPE1)
...
(check-expect
 (other-fun WS-TYPE1 "A" 3)
 WS-TYPE2)

(check-expect
 (other-fun WS-TYPE3 "S" 3)
 WS-TYPE3)


(define (other-fun ws 1s len)
  (if (< (string-length (wordle-current ws)) len)
      (make-wordle (wordle-past ws) (string-append
                                     (wordle-current ws)
                                     (string-upcase 1s)))
      ws))   
...
(define (end-wordle? ws guess tries)
  (or
   (string-in-list? guess (wordle-past ws))
   (= tries (length (wordle-past ws)))))
...
(define BG (square 200 "solid" "gray"))
(define FONT-SIZE 30)
...
(check-expect (draw-end-wordle WS-WORD1 "CAT" 1)
              (place-image (text
                            (string-append "it took you 1 tries!") 20 "black")
                           100 120
                           (overlay (text "you won :D" FONT-SIZE "green")
...
(check-expect (draw-end-wordle WS-TYPE-BAD "CAT" 1)
              (overlay (text "you lost :,(" FONT-SIZE "red")
                       BG)) 
     
                                 
(define (draw-end-wordle ws correct guess)
  (local [;win? : WS String -> Boolean
          ;did the person win the game?
          (define (win? ws correct)
            (string-in-list? correct (wordle-past ws)))]
    (if (win? ws correct)
        (place-image (text
                      (string-append "it took you "
...
(check-expect (draw-wordle WS-START "CATS" 4)
              (above
               GAP
...
(check-expect (draw-wordle WS-TYPE1 "CAT" 3)
              (above
               GAP
               (partial-guess->image "C" 3)
               (above
                GAP
                (partial-guess->image "" 3)
                (above
...
(define (draw-wordle ws correct guesses)
  (stack/v (append (draw-prior ws correct)
                   (draw-current ws guesses)
                   (draw-remaining ws guesses))))  
                    
                                                                          
; draw-current : WordleState Nat -> [List-of Image]
; visualizes the current guess
(check-expect (draw-current WS-START 5)
              (list
               (partial-guess->image "" 5)))

(check-expect (draw-current WS-WORD1 3)
              (list
               (partial-guess->image "" 3)))

(check-expect (draw-current WS-TYPE1 3)
              (list
               (partial-guess->image "C" 3)))


(define (draw-current ws guesses)
  (list (partial-guess->image (wordle-current ws) guesses))) 


; draw-prior : WordleState String -> [List-of Image]
; visualizes the prior guesses
(check-expect (draw-prior WS-WORD1 "CAT")
              (list
               (beside GAP
                       ...
                       (lsp->image (make-pair "A" LS-RIGHT))
                       GAP
                       (lsp->image (make-pair "T" LS-RIGHT))
                       GAP)))
(check-expect (draw-prior WS-START "CAT")
              '())
(check-expect (draw-prior WS-WORD2 "CAT")
              (list
               (beside GAP
                       (lsp->image (make-pair "C" LS-RIGHT))
                       GAP
...
(define (draw-prior ws correct)
  (local [;lsp-list: WordleState String -> [List-of LetterStatusPair]
          ;creates a list of LetterStatusPair
          (define (lsp-list ws correct)
            (map (λ (word)
                   (score word correct)) (wordle-past ws)))]
    (map (λ (x) (row->image (map (λ (y)
                                   (make-pair y lsp->image)) x)))
         (lsp-list ws correct))))    
 
  
 
; draw-remaining : WordleState Nat -> [List of Image]
; visualizes the remaining guesses

(check-expect (draw-remaining WS-WORD3 3)
              '())
(check-expect (draw-remaining WS-WORD1 3)
              (list
               (partial-guess->image "" 3)
               (partial-guess->image "" 3)))

(check-expect (draw-remaining WS-TYPE1 4)
              (list
               (partial-guess->image "" 4)
               (partial-guess->image "" 4)
               (partial-guess->image "" 4)
...
(define (draw-remaining ws guesses)
  (build-list (- guesses (length (wordle-past ws)))
              (λ (a) (partial-guess->image "" guesses))))   
